...
Mat.prototype.toArrayData = function(){
	var tempData = this.data,
		num = this.col * this.channel;
	var txt = new Array();
	for(var i = 0; i < this.row; i++){
		for(var j = 0; j < num; j++){
			txt[i * num + j] = tempData[i * num + j];
		}
	}
	return txt;
};
...
var imdiff = function(__src1, __src2, __dst){
	...
	for(var i = 0; i < height; i++){
        for(var j = 0; j < num; j++){
            dData[i * num + j] = Math.abs(s1Data[i * num + j] - s2Data[i * num + j]);
        }
    }
...
var imlabelp = function(__src1, __src2){
	//var dts = [44.2857,40.9592,12.0000,54.7727,40.8636,11.0000,90.7333,41.8667,11.0000,72.5000,42.0000,1.0000,77.0000,45.5000,3.0000];
	
	//return dts;
	
	(__src1) || error(arguments.callee, IS_UNDEFINED_OR_NULL/* {line} */);
	var height1 = __src1.row,
		width1 = __src1.col,		
		channel1 = __src1.channel;	
	
	(__src2) || error(arguments.callee, IS_UNDEFINED_OR_NULL/* {line} */);
	var height2 = __src2.row,
		width2 = __src2.col,		
		channel2 = __src2.channel;	
		

	if(height1==height2 && width1==width2){
			var dst = imdiff(__src1, __src2);
			var rect1 = [0, 0, 0, 0];
			var rect2 = [0, 0, 0, 0];
			var info={mat_dst:dst,
			mat_rect1:rect1,
			mat_rect2:rect2};
			return info;
	}
	
	var matr1 = cv.cvtColor(__src1, CV_RGBA2GRAY);
	var matr2 = cv.cvtColor(__src2, CV_RGBA2GRAY);
	...
	var min_r1 = -1, max_r1 = -1;
	var min_c1 = -1, max_c1 = 0;
	for(i = 0; i < r1; i++){
		for(j = 0; j < c1; j++){
			if(A1[c1*i+j] < 1){
				if(min_r1 == -1){
	...
	var rect1 = [min_c1, min_r1, max_c1-min_c1, max_r1-min_r1];
	
	var min_r2 = -1, max_r2 = -1;
	var min_c2 = -1, max_c2 = -1;
	for(i = 0; i < r2; i++){
		for(j = 0; j < c2; j++){
			if(A2[c2*i+j] < 1){
				if(min_r2 == -1){
	...
	var rect2 = [min_c2, min_r2, max_c2-min_c2, max_r2-min_r2];
	
	console.log(rect1);
	console.log(rect2);
	
	if(rect1[2] < rect2[2]){
		rect1[0] = rect1[0] - (rect2[2]-rect1[2]);
		rect1[2] = rect2[2];		
	}
	if(rect1[3] < rect2[3]){
		rect1[1] = rect1[1] - (rect2[3]-rect1[3]);
		rect1[3] = rect2[3];		
	}
	...
	for(i = 0; i < hm; i++){
        for(j = 0; j < num; j++){
            t1 = s1Data[(i+rect1[1]-1) * __src1.col * __src1.channel + (rect1[0]-1)*__src1.channel + j];
			t2 = s2Data[(i+rect2[1]-1) * __src2.col * __src2.channel + (rect2[0]-1)*__src2.channel + j];
			dData[i * num + j] = Math.abs(t1 - t2);
        }
    }
...
cv.imlabelp = imlabelp;

var imlabel = function(__src, __dst){
	...
	for(i = 1; i < r-1; i++){
		for(j = 1; j < c-1; j++){
			if(s[c*i+j] == 1 && tmp[c*i+j] == 0){
				tmp[c*i+j] = label;
				// 当前坐标入列  
				q.push(i, j);				
				queue_tail = queue_tail+1;
				while(queue_head != queue_tail){
					pix = [q[(queue_head-1)*2+0], q[(queue_head-1)*2+1]];
					// 8邻域搜索
					for(k = 0; k < 8; k++){
						pix1[0] = pix[0] + neighbour[2*k+0];
						pix1[1] = pix[1] + neighbour[2*k+1];
						if(pix1[0]>=1 && pix1[0]<=r-2 && pix1[1] >=1 && pix1[1]<=c-2){
							if(s[c*pix1[0]+pix1[1]] == 1 && tmp[c*pix1[0]+pix1[1]] == 0){
								// 如果当前像素邻域像素为1并且标记图像的这个邻域像素没有被标记，那么标记
								tmp[c*pix1[0]+pix1[1]] = label;
								q.push(pix1[0], pix1[1]);	
								queue_tail = queue_tail+1;
							}
						}
					}
	...
	for(i = 1; i <= max_num; i++){
		...
		xc = 0;
		yc = 0;
		xmin = xyi[0]; ymin = xyi[1];
		xmax = xyi[0]; ymax = xyi[1];
		for(j = 0; j < xyi.length; j++){
			if(j%2 == 0){
				// 是x
				xc = xc + xyi[j];
				if(xmin > xyi[j]){
		...
		xc = xc/xyi.length*2;
		yc = yc/xyi.length*2;
		var radius_x = (xmax - xmin)/2; 
		var radius_y = (ymax - ymin)/2; 
		if(radius_x > radius_y){
			radius = radius_x;
		}else{
			radius = radius_y;
		}
		dt.push(xc, yc, radius);
	}
	...
	for(i = 1; i <= max_num; i++){
		...
		flag = 0;
		di = new Array();
		di.push(dt[(i-1)*3], dt[(i-1)*3+1], dt[(i-1)*3+2]);
		for(j = 1; j <= max_num; j++){
			if(i == j){
				continue;
			}
			dj = new Array();
			dj.push(dt[(j-1)*3], dt[(j-1)*3+1], dt[(j-1)*3+2]);
			if(di[0]-di[2]>dj[0]-dj[2] && di[0]+di[2]<dj[0]+dj[2] && di[1]-di[2]>dj[1]-dj[2] && di[1]+di[2]<dj[1]+dj[2]){
				flag = 1;
				break;
			}			
		}
	...
	for(i = 1; i <= max_num; i++){
		...
		dts.push(dt[(i-1)*3], dt[(i-1)*3+1], dt[(i-1)*3+2]);
	}
	
	console.log(dts);
	for(i = 0; i < r; i++){
        for(j = 0; j < c; j++){
            dData[i * c + j] = sData[i * c + j];
        }
    }

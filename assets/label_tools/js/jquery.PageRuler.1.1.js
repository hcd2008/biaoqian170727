...
(function($) {
    $.pageRuler = function(params) {
        ...
        var f = {
            ...
            newv : function(t) {// 创建新的垂直参考线，有效宽度3像素
                ...
                $('<div class="zxxRefLine_v"></div>').appendTo(x).attr({
                    "id" : id,
                    "title" : t
                });
                return $("#" + id);
            ...
            newh : function(t) {// 创建新的垂直参考线，有效宽度3像素
                ...
                $('<div class="zxxRefLine_h"></div>').appendTo(x).attr({
                    "id" : id,
                    "title" : t
                });
                return $("#" + id);
    ...
    $(document).keyup(function(e) {
        ...
        if (e.keyCode === k["semi"] || e.keyCode === k["semi2"] || e.keyCode === k["h"]) {
            $.lineToggle();
        }
        if (e.keyCode === k["esc"]) {
            $.pageRulerHide();
        ...
        if (e.keyCode === k["right"]) {
            var lastV = "zxxRefLineV" + $(".zxxRefLine_v").size();
            var lastLeft = $("#"+lastV).css("left");
            
            var newLeft = parseInt(lastLeft, 10) + 5;
            $("#"+lastV).css("left", newLeft + "px").attr("title", newLeft + "px");;
        }
        if (e.keyCode === k["left"]) {
            var lastV = "zxxRefLineV" + $(".zxxRefLine_v").size();
            var lastLeft = $("#"+lastV).css("left");
        ...
        if (e.keyCode === k["up"]) {
            var lastH = "zxxRefLineH" + $(".zxxRefLine_h").size();
            var lastTop = $("#"+lastH).css("top");
            
            var newTop = parseInt(lastTop, 10) - 5;
            $("#"+lastH).css("top", newTop + "px").attr("title", newTop + "px");;;
        }
        if (e.keyCode === k["down"]) {
            var lastH = "zxxRefLineH" + $(".zxxRefLine_h").size();
            var lastTop = $("#"+lastH).css("top");

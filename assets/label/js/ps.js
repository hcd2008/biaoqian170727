...
var canvasrulex = document.getElementById('canvasrulex');
var ctxrulex = canvasrulex.getContext('2d');
var canvasruley = document.getElementById('canvasruley');
var ctxruley = canvasruley.getContext('2d');
...
img.onload = function () {
    ...
    canvasrule.height = h;
    canvasrulex.width = w;
    canvasrulex.height = 20;
    canvasruley.width = 20;
    canvasruley.height = h;
...
canvascur.onmouseup = function (e) {
    ...
    $("#resinfo").show();$("#qdesc").val(content);
                    }
                    guiji = [];
                }
                break;
...
function pick(x, y) {
    ...
    qs = 'rgba(' + data[0] + ',' + data[1] +
        ',' + data[2] + ',' + data[3] + ')';
    var recolor = 'rgba(' + (255 - parseInt(data[0])) + ',' + (255 - parseInt(data[1])) +
        ',' + (255 - parseInt(data[2])) + ',1)';

    ctxcur.fillStyle = qs;
    ctxcur.beginPath();
    ctxcur.arc(x, y, 45, 0, Math.PI * 2);
    ctxcur.stroke();
    ctxcur.fill();

    ctxcur.fillStyle = recolor;
    ctxcur.beginPath();
    ctxcur.fillText(data[0] + ',' + data[1] +
        ',' + data[2] + ',' + data[3], x - 45, y);
}
...
function ruleFix() {
    ...
    ctxrulex.strokeStyle = 'black';
    ctxrulex.beginPath();
    ctxrulex.fillText(0, 5, 10);
    ctxrulex.moveTo(0, 20);
    ctxrulex.lineTo(w, 20);
    var mt = 0;
    for (var i = 1; i <= ruleXNum; i++) {
        if (i % 5 == 0) {
            mt = 10;
            ctxrulex.fillText(i / 10, (i * pxMM - (i % 10 == 0 ? 5 : 10)), 10);
        } else {
            mt = 15;
        }

        ctxrulex.moveTo(i * pxMM, mt);
        ctxrulex.lineTo(i * pxMM, 20);
    }
    ...
    ctxruley.moveTo(20, 0);
    ctxruley.lineTo(20, h);
    for (var i = 1; i <= ruleYNum; i++) {
        if (i % 5 == 0) {
            mt = 10;
            ctxruley.fillText(i / 10, 0, (i * pxMM + 5));
        } else {
            mt = 15;
        }

        ctxruley.moveTo(mt, i * pxMM);
        ctxruley.lineTo(20, i * pxMM);
    }

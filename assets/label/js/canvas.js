...
function brush(num) {
    ...
    canvas.onmousedown = function(evt) {
        ...
        startX = (evt.pageX - this.offsetLeft) * scalePencent;
        startY = (evt.pageY - this.offsetTop) * scalePencent;
        flag = 1;
        // alert(mouseX+'——————————————'+mouseY);
        cxt.closePath();
        cxt.beginPath();
    ...
    canvas.onmousemove = function(evt) {
        evt = evt ? evt : window.event;
        endX = (evt.pageX - this.offsetLeft) * scalePencent;
        endY = (evt.pageY - this.offsetTop) * scalePencent;
        // console.log(endX);
...
function straw(num) {
    ...
    canvas.onmousedown = function(evt) {
        evt = evt ? evt : window.event;
        X = (evt.pageX - this.offsetLeft) * scalePencent;
        Y = (evt.pageY - this.offsetTop) * scalePencent;
...
function magnifier(scale) {
    ...
    canvas_bak.width = scaleX;
    canvas_bak.height = scaleY;
    cxt_bak.drawImage(img, 0, 0, canvas_bak.width, canvas_bak.height);
...
var draw_graph = function(graphType, style, obj) {
    ...
    if (graphTypes.indexOf(graphType) > -1) {
        ...
        var mousedown = function(e) {
            ...
            startX = e.offsetX; //(e.pageX - this.offsetLeft) * scalePencent; // canvasLeft;
            startY = e.offsetY; //(e.pageY - this.offsetTop) * scalePencent; // canvasTop;
        ...
        var mouseup = function(e) {
            // e=e||window.event;
            e = e ? e : window.even;
            //            var xx = (e.pageX - this.offsetLeft) * scalePencent;// canvasLeft;
            //            var yy = (e.pageY - this.offsetTop) * scalePencent;// canvasTop;
            ...
            if (graphType == 'text' || graphType == 'linetext' || graphType == 'squaretext' || graphType == 'circletext') {
                var content = window.prompt('请输入问题描述', '');
                $("#qdesc").val(content);
                $("#resinfo").show();
                //增加注释
                if (content) {
        ...
        var mousemove = function(e) {
            ...
            var o_x = e.pageX;
            var o_y = e.pageY;
            var x = e.offsetX; //(e.pageX - this.offsetLeft) * scalePencent; // canvasLeft;
            var y = e.offsetY; //(e.pageY - this.offsetTop) * scalePencent; // canvasTop;
            //console.log(x + " " + y);
